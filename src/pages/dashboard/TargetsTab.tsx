...
interface EmployeeTarget {
  ...
  orders_target: number; new_customers_target: number; collections_target: number; is_locked: boolean
}
...
const KPI_FIELDS: { key: keyof EmployeeEdit; label: string; unit: string }[] = [
  ...
  { key: 'collections_target', label: 'التحصيل', unit: 'جنيه' },
]
...
export default function TargetsTab({ month: selMonth, year: selYear }: TargetsTabProps) {
  ...
  const [viewMode, setViewMode] = useState<'managers' | 'all'>('managers')
  ...
  const filteredEmployees = useMemo(() => {
    ...
    if (targetFilter === 'بدون أهداف') {
      result = result.filter(e => {
        const t = employeeTargetMap[e.employee_id]
        return !t || (t.sales_target === 0 && t.visits_target === 0 && t.orders_target === 0 && t.new_customers_target === 0 && t.collections_target === 0)
      })
    } else if (targetFilter === 'لديه أهداف') {
      result = result.filter(e => {
        const t = employeeTargetMap[e.employee_id]
        return t && (t.sales_target > 0 || t.visits_target > 0 || t.orders_target > 0 || t.new_customers_target > 0 || t.collections_target > 0)
      })
    ...
    return result
  }, [employees, roleFilter, searchTerm, targetFilter, employeeTargetMap])
      ...
      if (!empTargetResult.error && empTargetResult.data) {
        ...
        for (const et of list) {
          map[et.employee_id] = {
            ...
            new_customers_target: et.new_customers_target || 0,
            collections_target: et.collections_target || 0, is_locked: et.is_locked || false,
          }
        }
        ...
        for (const et of list) {
          edits[et.employee_id] = {
            ...
            collections_target: (et.collections_target || 0).toString(),
          }
        }
    ...
    if (!employeeEdits[emp.employee_id]) {
      setEmployeeEdits(prev => ({
        ...
        [emp.employee_id]: {
          ...
          sales_target: '0', visits_target: '0', orders_target: '0', new_customers_target: '0', collections_target: '0',
        },
      }))
    }
  }
  ...
  const handleSaveEmployee = useCallback(async (empId: string) => {
    ...
    const { error } = await targetService.upsertEmployeeTarget(
      ...
      parseFloat(edit.collections_target) || 0, token
    )
    ...
    setEmployeeTargetMap(prev => ({
      ...
      [empId]: {
        ...
        new_customers_target: parseFloat(edit.new_customers_target) || 0,
        collections_target: parseFloat(edit.collections_target) || 0, is_locked: false,
      },
    }))
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center gap-2 mb-3">
        <button onClick={() => setViewMode('managers')}
          className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
            viewMode === 'managers' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600'
          }`}>مدراء البيع</button>
        <button onClick={() => setViewMode('all')}
          className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
            viewMode === 'all' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600'
          }`}>جميع الموظفين</button>
      </div>
                                    ...
                                    <div key={field.key}>
                                      ...
                                      <input type="number"
                                        ...
                                        onChange={e => setEmployeeEdits(prev => ({
                                          ...
                                          orders_target: '0', new_customers_target: '0', collections_target: '0',
                                            }),
        ...
        <div>
          <div className="flex flex-col gap-2 mb-3">
            <input type="text" placeholder="بحث بالاسم أو الكود" value={searchTerm}
              ...
              className="w-full border border-gray-200 rounded-lg p-2 text-xs text-gray-800 text-right bg-white" />
            <div className="flex gap-2">
              <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
                className="flex-1 border border-gray-200 rounded-lg px-2 py-2 text-xs text-gray-800 bg-white cursor-pointer">
          ...
          <div className="space-y-2">
            {filteredEmployees.map(emp => {
              const expanded = expandedManagers[emp.employee_id]
              return (
                <div key={emp.employee_id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  <button onClick={() => { ensureEdit(emp); toggleManager(emp.employee_id) }}
                    className="w-full flex items-center justify-between p-3 active:bg-gray-50 transition-colors cursor-pointer">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className={`text-xs shrink-0 transition-transform text-gray-400 ${expanded ? 'rotate-90' : ''}`}>▶</span>
                      <div className="text-right min-w-0">
                        <p className="text-[9px] text-gray-400 font-mono">{emp.employee_code}</p>
                        <p className="text-xs font-bold text-gray-800 truncate">{emp.employee_name}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={`text-[9px] px-2 py-0.5 rounded-full font-semibold ${roleBadgeColor(emp.role_type)}`}>
                        {emp.role_type}
                      </span>
                    ...
                    <div className="border-t border-gray-100 px-4 pb-4 pt-3 space-y-3">
                      {KPI_FIELDS.map(field => (
                        <div key={field.key}>
                          <label className="text-[10px] text-gray-500 block mb-1">{field.label} ({field.unit})</label>
                          <input type="number"
                            value={employeeEdits[emp.employee_id]?.[field.key] || '0'}
                            onChange={e => setEmployeeEdits(prev => ({
                              ...prev,
                              [emp.employee_id]: { ...prev[emp.employee_id], [field.key]: e.target.value },
                            }))}
                            className="w-full border border-gray-200 rounded-lg p-2 text-sm text-gray-800 text-right bg-white" />
                        </div>
                      ))}
                      {employeeSuccess === emp.employee_id && (
                        <div className="bg-green-50 border border-green-200 text-green-700 text-xs rounded-lg p-2">تم الحفظ</div>
                      )}
                      {employeeError && savingEmployee === emp.employee_id && (
                        <div className="bg-red-50 border border-red-200 text-red-700 text-xs rounded-lg p-2">{employeeError}</div>
                      )}
                      <button onClick={() => handleSaveEmployee(emp.employee_id)}
                        disabled={savingEmployee === emp.employee_id}
                        className="w-full py-2 bg-indigo-600 text-white rounded-xl text-xs font-semibold active:opacity-80 disabled:opacity-50 transition-colors cursor-pointer">
                        {savingEmployee === emp.employee_id ? 'جاري الحفظ...' : 'حفظ الهدف'}
                      </button>
                    </div>
                  )}
                </div>
            ...
            {filteredEmployees.length === 0 && (
              <p className="text-center py-8 text-gray-400 text-xs">لا يوجد موظفون مطابقون</p>
            )}
          </div>
        </div>
      )}
