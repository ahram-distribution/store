...
export function FlashOffersManagementPage() {
  const canManage = useCapability('flash_offers.manage')
  ...
  const fetchOffers = async () => {
    ...
    const { data } = await supabase.rpc('get_governed_flash_offers', { p_token: token })
    if (data) setOffers(Array.isArray(data) ? data : [])
    setLoading(false)
  }
  ...
  function resetCreate() {
    ...
    for (const col of FLASH_OFFER_COLUMNS) {
  ...
  function validateOffer(data: Record<string, any>, isCreate: boolean): string | null {
  ...
  async function handleCreateSave() {
    if (!canManage) return
    const err = validateOffer(createForm, true)
    if (err) { toast.error(err); return }
    setSaving(true)
    const token = getToken()
    try {
      const { error } = await supabase.rpc('governed_create_flash_offer', {
        p_token: token,
  ...
  async function handleEditSave() {
    ...
    const err = validateOffer(editForm, false)
    if (err) { toast.error(err); return }
    setSaving(true)
    const token = getToken()
    try {
      ...
      const { error: updateErr } = await supabase.rpc('governed_update_flash_offer', payload)
  ...
  async function handleStatusAction(action: string) {
    ...
    setSaving(true)
    const token = getToken()
    const fn = action === 'activate' ? 'governed_activate_flash_offer' : 'governed_cancel_flash_offer'
    ...
    <div className="space-y-4 pb-24">
      ...
      {showCreate && canManage && (
        <div className="space-y-4">
          <DynamicSchemaEditor
            title="عرض ساعة جديد"
            columns={FLASH_OFFER_COLUMNS.filter(c => c.key !== 'status' && c.key !== 'available_quantity')}
            data={createForm}
      ...
      {!editingId && !showCreate && (
        <>
          ...
          <div className="space-y-2">
            {offers.map((offer) => (
              <div key={offer.id} className={`bg-white rounded-lg border border-border p-3 ${editingId === offer.id ? 'ring-2 ring-primary' : ''}`}>
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  {canManage && (offer.status === 'draft' || offer.status === 'scheduled') && (
                    <button onClick={async () => {
                      const token = getToken()
                      const { error } = await supabase.rpc('governed_activate_flash_offer', { p_token: token, p_id: offer.id })
                      if (error) toast.error(error.message); else { toast.success('تم التفعيل'); fetchOffers() }
                  ...
                  {canManage && offer.status === 'active' && (
                    <button onClick={async () => {
                      const token = getToken()
                      const { error } = await supabase.rpc('governed_cancel_flash_offer', { p_token: token, p_id: offer.id })
                      if (error) toast.error(error.message); else { toast.success('تم الإلغاء'); fetchOffers() }
      ...
      {editingId && (
        <div className="space-y-4">
          <DynamicSchemaEditor
            title="تعديل عرض الساعة"
            columns={FLASH_OFFER_COLUMNS}
            data={editForm}
