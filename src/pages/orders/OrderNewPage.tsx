...
function mapProduct(row: any): ProductWithPrice {
  ...
  return {
    ...
    salesBlocked: unitPrices.length === 0 || (row.is_out_of_stock === true && row.is_active !== false),
...
export function OrderNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    // ── OPEN WHATSAPP — uses unified order RPC (same Source of Truth as order detail page) ──
    try {
