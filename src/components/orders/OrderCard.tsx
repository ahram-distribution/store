...
export function OrderCard({ order, onClick }: OrderCardProps) {
  ...
  const addressParts = [order.customer_area, order.governorate].filter(Boolean)
  ...
  return (
    ...
    >
      ...
      <p className="text-base font-bold text-text mb-1.5 leading-snug">
      ...
      {(order.delivery_mode || order.governorate || order.collection_badge) && (
        <div className="flex items-center gap-1.5 mt-1.5 pt-1.5 border-t border-border/50">
          {order.governorate && !order.customer_area && (
