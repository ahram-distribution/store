    ...
    >
      ...
      <div className="flex items-center justify-between mt-1.5">
        <div className="text-[11px] text-text-secondary space-y-0.5">
          {order.owner_name && (
            <p className="truncate max-w-[160px]" title={order.owner_name}>{order.owner_name}</p>
          )}
          {order.creator_name && order.creator_name !== order.owner_name && (
            <p className="truncate max-w-[160px] text-primary/70">{order.creator_name}</p>
          )}
          <p>{dateStr} {timeStr}</p>
        </div>
