...
export function OrderDetailView({ data, actions, onBack, editMode, editItems, onQuantityChange, onRemoveItem, onPriceChange, onUnitChange, unitOptions, onDeleteSelected, onAddProduct, editActions, shortageProductIds, inventorySnapshot, businessStatusByItem, eventLog }: OrderDetailViewProps) {
  ...
  const hasNetItems = useMemo(() => items.length > 0 && items.some((i: any) => {
      ...
      <div className="bg-white rounded-lg border border-[#E5E7EB] shadow-sm p-4 space-y-2 text-[13px]">
        ...
        <div>
          ...
          <span className="inline-flex items-center gap-1.5">
            ...
            {(customer?.phone || order.snapshot_customer_phone) && (() => {
              const phone = (customer?.phone || order.snapshot_customer_phone)!;
              const digits = phone.replace(/[^0-9]/g, '')
              return (
                <span className="inline-flex items-center gap-1">
                  <a href={`tel:${phone}`} title="اتصال" className="inline-flex items-center gap-1 text-[11px] text-[#2563EB] bg-[#EFF6FF] hover:bg-[#DBEAFE] px-2 py-1 rounded-md transition-colors font-medium">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                    اتصال
                  </a>
                  {digits && (
                    <a href={`https://wa.me/${digits}`} target="_blank" rel="noopener noreferrer" title="واتساب" className="inline-flex items-center gap-1 text-[11px] text-[#059669] bg-[#ECFDF5] hover:bg-[#D1FAE5] px-2 py-1 rounded-md transition-colors font-medium">
