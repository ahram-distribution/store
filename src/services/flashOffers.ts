...
function mapRow(row: any): FlashOfferRecord {
  return {
    id: row.id,
    title: row.title,
...
export const flashOfferService = {
  async getAll(): Promise<FlashOfferRecord[]> {
    const token = getToken()
    if (!token) return []
    const { data, error } = await supabase.rpc('get_governed_flash_offers', { p_token: token })
    if (error) throw error
    return (data ?? []).map(mapRow)
  },

  async getActive(): Promise<FlashOfferRecord[]> {
    const token = getToken()
    const { data, error } = await supabase.rpc('get_governed_active_flash_offers', { p_token: token })
    if (error) throw error
    return (data ?? []).map(mapRow)
  },
  ...
  async create(params: {
    ...
    const { data, error } = await supabase.rpc('governed_create_flash_offer', {
      p_token: token,
  ...
  async activate(id: string): Promise<{ success: boolean; error?: string }> {
    const token = getToken()
    if (!token) return { success: false, error: 'Not authenticated' }
    const { data, error } = await supabase.rpc('governed_activate_flash_offer', { p_token: token, p_id: id })
    if (error) return { success: false, error: error.message }
  ...
  async cancel(id: string, reason?: string): Promise<{ success: boolean; error?: string }> {
    const token = getToken()
    if (!token) return { success: false, error: 'Not authenticated' }
    const { data, error } = await supabase.rpc('governed_cancel_flash_offer', {
      p_token: token,
      p_id: id,
