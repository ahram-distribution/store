-- ============================================================================
-- Customer Report — align get_governed_customers order stats with the
-- approved status model (20270908_activity_and_inventory_approval_rules).
--
-- Problem: the ps lateral (order statistics) and the p_no_orders filter
-- counted ALL orders regardless of status, so draft, submitted (طلب شراء),
-- sales_manager_approved (موافقة مدير البيع), returned_for_revision (معاد
-- للتعديل), and cancelled (ملغى) orders inflated:
--   - previous_order_count / previous_orders_total / last_order_number /
--     last_order_date / last_order_total
--   - the "بدون طلبات" (no orders) filter
-- These statuses must be EXCLUDED from statistics; eligibility starts at
-- approved (معتمد) and includes reviewing (تم القيد بالسيستم) and all later
-- post-approval statuses. The canonical rule is public.is_order_in_statistics().
--
-- Delivered semantics are unchanged: delivered_total still means current
-- status = 'delivered' (منفذ فعليًا), which is within statistics and the
-- execution group, so the existing FILTER expression needs no change.
--
-- The change applies to the shared RPC, so every consumer (customer report
-- PDF/Excel/print, customers list, balance display) respects the same rule.
-- current_balance = ps.orders_total - collected now also excludes
-- non-statistical orders, matching the canonical model.
-- ============================================================================
...
COMMENT ON FUNCTION public.get_governed_customers IS 'قائمة العملاء مع دعم البحث والنطاق الهرمي والفلاتر الزمنية + حقول تقرير العملاء. إحصائيات الطلبات (العدد/الإجمالي/آخر طلب) وفلتر "بدون طلبات" تعتمد على is_order_in_statistics — تبدأ الأهلية من معتمد (approved) وتشمل تم القيد بالسيستم (reviewing) وكل ما بعده، وتستبعد draft/submitted/sales_manager_approved/returned_for_revision/cancelled.';
