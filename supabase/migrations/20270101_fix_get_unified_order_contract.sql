-- ============================================================================
-- Migration 20270101: توحيد عقد get_unified_order — إزالة جميع الـ overloads
-- ============================================================================
-- PUBLIC RPC CONTRACT RULE (معتمد كنمط معماري للمشروع)
-- ============================================================================
-- أي RPC يتم استدعاؤه من Frontend أو PostgREST يجب أن يستقبل:
--
--   p_token text
--
-- وليس uuid.
--
-- ويتم تحويله مرة واحدة فقط داخل الدالة إلى:
--
--   v_token uuid := p_token::uuid
--
-- ثم يستخدم v_token داخليًا فقط.
--
-- السبب:
--   PostgREST يُرسل جميع الباراميترات كنصوص (JSON → text)
--   وجود overloads متعددة (p_token uuid) و (p_token text) يسبب 404/400
--   لأن PostgREST يفشل في تحديد أي overload يستخدم
--
-- Public API          Boundary (cast واحد)     Internal Logic
-- ──────────────────────────────────────────────────────────────
-- p_token text  ──►  v_token uuid := ...  ──►  WHERE token = v_token
--                                              get_visible_employee_ids(v_token)
--                                              is_upper_management(v_session.employee_id)
--                                              ... (uuid فقط)
--
-- يحظر إنشاء أي RPC جديد أو Migration يستقبل p_token uuid مباشرة.
-- ============================================================================
-- الإصلاح:
--   1. حذف جميع overloads القديمة للدالة
--   2. إنشاء تعريف واحد فقط: get_unified_order(p_token text, p_id uuid)
--   3. تطبيق الـ Contract Rule: تحويل واحد p_token::uuid عند البوابة
-- ============================================================================
...
-- ============================================================================
-- 2. الإنشاء الموحد — Signature واحد فقط (text, uuid)
-- ============================================================================
...
COMMENT ON FUNCTION public.get_unified_order IS 'مصدر الحقيقة الموحد للطلب — Signature واحد: (p_token text, p_id uuid)';
