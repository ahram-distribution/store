-- ============================================================================
-- 20271115 — Restore o.reference_number in get_unified_order (الرقم المرجعى)
-- ----------------------------------------------------------------------------
-- BUSINESS BUG: Order Details showed "الرقم المرجعى: —" while the Order Card
-- showed the stored reference (e.g. ORD-2026-000313) for the same order.
--
-- Root cause: get_unified_order was re-created by 20271106_supreme_edit_base_price.sql
-- (to expose order_items.base_unit_price) and that rewrite dropped
-- 'reference_number' from the order jsonb. The plural RPC get_unified_orders
-- (used by the Orders list) always returned 'reference_number', so the list
-- stayed correct while OrderDetailView (which reads data.order.reference_number)
-- received undefined.
--
-- Fix: recreate get_unified_order from the current 20271106 definition with
-- ONE added header field: 'reference_number', o.reference_number — the SAME
-- authoritative column that the list reads and that is set once when the order
-- transitions submitted -> reviewing. No new numbering, no duplicated field.
--
-- The order object is split into two jsonb_build_object calls concatenated
-- with || because the full 51-key list exceeds PostgreSQL's 100-argument
-- variadic limit (error 54023), which is why the field keeps being lost on
-- rewrites of this function.
-- ============================================================================
...
COMMENT ON FUNCTION public.get_unified_order IS 'مصدر الحقيقة الموحد للطلب — يعرض الطلب مع العميل والأصناف (بما فيها base_unit_price) والتوصيل والتحصيل والمرتجعات والرقم المرجعي';