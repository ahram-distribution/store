-- ============================================================
-- Migration: Fix Collections source in get_governed_target_performance
--
-- Phase: 5.2 — Commit A (Collections)
-- Cause: Used c.created_at instead of c.collected_at + status filter
--        per canonical spec (collected_at + status='collected')
--
-- Changes:
--   employee_collections CTE:
--     WHERE c.created_at BETWEEN → WHERE c.collected_at BETWEEN
--     Added: AND c.status = 'collected'
-- ============================================================

CREATE OR REPLACE FUNCTION public.get_governed_target_performance(p_token uuid, p_month integer DEFAULT NULL::integer, p_year integer DEFAULT NULL::integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
