-- ============================================================================
-- P1 HOTFIX: column reference "v_item" is ambiguous in governed_create_order
-- 
-- Root Cause (20260922_product_out_of_stock.sql):
--   Line 165: v_item jsonb;                                           ← PL/pgSQL variable
--   Line 202: jsonb_array_elements(p_items) AS v_item                 ← table alias (SAME NAME)
--   Line 203: ON p.id = (v_item->>'product_id')::uuid                 ← WHICH v_item?
--
--   PostgreSQL cannot distinguish between the PL/pgSQL variable and the
--   column alias, producing: column reference "v_item" is ambiguous
--
-- Fix: rename the alias from v_item → elem to avoid the collision.
-- ============================================================================
