-- ============================================================================
-- Migration: get_unified_order — expose persisted order_type (نوع الطلب)
--
-- BUSINESS BUG: Order Details showed "نقدي" while Orders List showed "آجل"
-- for the same order. Root cause: the singular RPC get_unified_order
-- (used by OrderDetailPage) omitted 'order_type' from its order jsonb, so
-- OrderDetailView read undefined and the `=== 'credit'` check fell through
-- to the 'نقدي' branch. The plural RPC get_unified_orders (used by OrdersPage)
-- always returned 'order_type', so the list was correct.
--
-- Fix: return the SAME persisted column the list reads — o.order_type
-- ('cash' | 'credit', CHECK-constrained) — nothing inferred, defaulted, or
-- recalculated. Body is the exact current definition (20270809_order_audit_trail_fix.sql)
-- with a single added header field.
-- ============================================================================
