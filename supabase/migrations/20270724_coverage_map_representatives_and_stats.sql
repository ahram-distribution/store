-- ================================================================
-- Coverage Map: Sales Representatives Live Tracking + Governorate Stats
--
-- Changes to get_coverage_map:
--   1. representative_location CTE — replaces last_location,
--      adds visit check-in GPS fallback
--   2. employee_region CTE — resolves employee governorate from
--      nearest reference_city to their current location
--   3. governorate_stats CTE — aggregates customers + employees
--      by governorate
--   4. Employee JSON: added governorate field
--   5. Result JSON: added governorate_stats section
--   6. covered_governorates: now includes employee governorates
-- ================================================================
...
BEGIN
    ...
    -- ============================================================
    -- REPRESENTATIVE LOCATION — tracking with visit check-in fallback
    -- Priority: latest tracking point > latest visit check-in GPS
    -- ============================================================
    ...
    -- ============================================================
    -- EMPLOYEE REGION — resolves governorate from current location
    -- by matching against nearest reference_city coordinates
    -- ============================================================
    employee_region AS (
        SELECT DISTINCT ON (rl.employee_id)
            rl.employee_id,
        ...
        JOIN active_sessions s ON s.employee_id = rl.employee_id
    ...
    SELECT jsonb_build_object(
        ...
        'employees', COALESCE((SELECT jsonb_agg(jsonb_build_object(
            ...
            'last_seen_at', CASE
                WHEN la.last_activity_at IS NOT NULL AND rl.location_at IS NOT NULL
        ...
        LEFT JOIN representative_location rl ON rl.employee_id = as2.employee_id
        LEFT JOIN employee_region er ON er.employee_id = as2.employee_id
...
COMMENT ON FUNCTION public.get_coverage_map IS
  'خريطة التغطية — العملاء (3 مستويات) + المندوبين (تتبع حي) + إحصائيات المحافظات';
