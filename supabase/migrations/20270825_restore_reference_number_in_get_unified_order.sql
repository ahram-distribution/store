-- ============================================================================
-- Migration: get_unified_order — expose persisted reference_number (الرقم المرجعى)
--
-- BUSINESS BUG: Order Details showed "الرقم المرجعى: —" while Order Card showed
-- the manually-entered reference number (e.g. 1528) for the same order.
-- Root cause: the singular RPC get_unified_order (used by OrderDetailPage)
-- omitted 'reference_number' from its order jsonb, so OrderDetailView read
-- undefined. The plural RPC get_unified_orders (used by OrdersPage) always
-- returned 'reference_number', so the list was correct.
--
