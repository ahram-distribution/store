-- ============================================================================
-- Phase C2: Apply Business-Approved Weight Policy
-- DATE: 2026-12-02
--
-- WHAT:
--   1. Set approved weights in performance_weights_config
--   2. Update get_governed_target_performance to use get_effective_weights
--
-- APPROVED WEIGHTS (Owner decision):
--   Sales = 75, New Customers = 10, Orders = 7.5, Visits = 7.5,
--   Collections = 0, Attendance = 0
--
-- Since approved values match the previous hardcoded values, ALL scores
-- remain identical. Only the SOURCE changes (config → get_effective_weights).

-- NOTE: The existing check constraint ck_weights_sum checks only 5 columns.
-- With 6 KPIs (including orders), we must drop it and recreate with all 6.
-- ============================================================================

-- ============================================================================
-- STEP 1: Drop old constraint, recreate with orders_weight_percent
-- ============================================================================
ALTER TABLE public.performance_weights_config
  DROP CONSTRAINT IF EXISTS ck_weights_sum;

ALTER TABLE public.performance_weights_config
  ADD CONSTRAINT ck_weights_sum
  CHECK (
...
-- ============================================================================
-- STEP 2: Set approved weights in performance_weights_config
-- ============================================================================
UPDATE public.performance_weights_config
SET
    sales_weight_percent = 75,
    orders_weight_percent = 7.5,
    visits_weight_percent = 7.5,
    new_customers_weight_percent = 10,
    collections_weight_percent = 0,
    attendance_weight_percent = 0
WHERE target_year = 2026;

-- ============================================================================
-- STEP 3: Update get_governed_target_performance
-- Replace hardcoded weight variables with get_effective_weights per employee
-- Company section reads from performance_weights_config
-- ============================================================================
...
BEGIN
    ...
    -- Read company-level weights from config (fallback to DECLARE defaults)
    ...
    IF FOUND THEN
        ...
        v_comp_new_customers_weight := v_company_weights_record.new_customers_weight_percent;
        v_comp_collections_weight := v_company_weights_record.collections_weight_percent;
        v_comp_attendance_weight := v_company_weights_record.attendance_weight_percent;
    END IF;

    -- Company targets = SUM of all active employees' targets
    SELECT
    ...
    -- Company overall weighted score (from config values)
