-- =============================================================
-- Fix GPS drift in get_employee_day_map distance calculation
-- Adds 3-layer GPS drift protection:
--   1. Accuracy filter: skip points with accuracy_meters > threshold
--   2. Min distance: only count segments > threshold as real movement
--   3. Speed outlier: filter segments requiring implausible speed
-- =============================================================

DROP FUNCTION IF EXISTS public.get_employee_day_map(p_token uuid, p_employee_id uuid, p_date date);
...
BEGIN
    ...
    -- Build route array with drift-filtered distance calculation
    FOR v_pt IN
    ...
    LOOP
        ...
        IF v_i = 1 THEN
            ...
            IF v_pt.accuracy_meters IS NULL OR v_pt.accuracy_meters <= p_max_accuracy THEN
                ...
                v_anchor_set := true;
            END IF;
        ELSIF v_anchor_set THEN
            -- Filter 1: low-accuracy points are excluded from distance
