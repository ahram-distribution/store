-- ============================================================================
-- CORRECTIVE: Remove erroneous get_governed_products uuid overload
--
-- Root cause: Phase 4 (20270802_inventory_governance_phase4.sql) used
-- CREATE OR REPLACE with p_token uuid, which did not replace the existing
-- p_token text function but instead created a duplicate overload.
-- This caused PostgREST ambiguity for all callers.
--
-- Fix:
--   1. DROP the erroneous uuid overload
--   2. REPLACE the canonical text overload to add inventory governance fields
-- ============================================================================

-- 1. Remove the erroneous Phase 4 overload (uuid first arg)
DROP FUNCTION IF EXISTS public.get_governed_products(
  uuid, boolean, boolean, text, uuid, boolean
);

-- 2. Replace canonical text overload — add 3 inventory governance fields
--    Preserves: signature, pagination, price guard, recently_available_at,
--    all WHERE conditions, all existing response fields.
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_governed_products IS 'المنتجات مع دعم البحث والتصفية والترقيم — يمنع ظهور الأصناف بدون سعر لواجهة العملاء — يشمل recently_available_at وحقوق الحوكمة';
