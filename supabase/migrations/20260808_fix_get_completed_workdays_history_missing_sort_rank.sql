-- ============================================================================
-- Fix: get_completed_workdays_history — missing sort_rank in CTE output
-- Root Cause:
--   paginated_employee_ids CTE (line 1174) selects from paginated_employees
--   but does NOT include sort_rank in its output columns.
--   The outer ORDER BY at line 1294 references pei.sort_rank which doesn't
--   exist in the CTE output, causing PostgreSQL error 42703.
--
--   Previous version (20260727) used ORDER BY pei.total_net_minutes DESC
--   which was a valid column. The rewrite (20261001) changed to sort_rank
--   but forgot to include it in the CTE SELECT list.
-- ============================================================================
...
BEGIN
    ...
    -- FIX: added pe.sort_rank to resolve error 42703 (column pei.sort_rank does not exist)
