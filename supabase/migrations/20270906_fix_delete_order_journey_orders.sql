-- ============================================================================
-- FIX: DELETE ORDERS INSIDE DELIVERY JOURNEYS — journey_orders FK dependency
-- ============================================================================
-- Reported failure (ORD-2026-000154):
--
--   Failed to delete order:
--   update or delete on table "orders" violates foreign key constraint
--   "journey_orders_order_id_fkey" on table "journey_orders"
--
-- Root cause:
--   journey_orders.order_id REFERENCES orders(id) WITHOUT ON DELETE CASCADE
--   (created in 20260810_delivery_journey_model.sql). The order-deletion RPCs
--   never removed the order's journey-membership rows, so any order belonging
--   to a delivery journey failed to delete.
--
-- Surgical fix (no redesign):
--   Each order-deletion path deletes the order's journey_orders rows before
--   deleting the order:
--
--     journey_orders -> ... -> orders
--
--   The journey itself is untouched: journey_events (journey-level) and the
--   delivery_journeys row remain intact; only the deleted order's membership
--   is removed. delivery_tracking rows for the order are already removed in
--   each path (their journey_id FK is released with them).
--
--   Scope: all three order-deletion entry points share this dependency:
...
--        also gains a preview count so the center reports journey membership)
--
