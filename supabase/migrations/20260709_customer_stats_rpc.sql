-- ============================================================================
-- Migration 20260709: Add customer statistics to unified order RPCs
-- ============================================================================
-- Adds to BOTH get_unified_orders and get_unified_order:
--   customer_display_address  — pre-formatted from customer_addresses
--   customer_order_count      — total non-deleted orders per customer
--   customer_lifetime_total   — sum(total_amount) across all orders
--   customer_last_order_date  — most recent order created_at
--   customer_last_order_number — most recent order number
--
-- Uses a single-pass CTE (customer_stats) — no LATERAL, no N+1.
-- All new fields are nullable. No existing fields changed.
-- ============================================================================

-- ============================================================================
-- 1. Composite index for customer_stats aggregation
-- ============================================================================
-- Supports GROUP BY customer_id + ORDER BY created_at DESC inside array_agg
CREATE INDEX IF NOT EXISTS idx_orders_customer_created_at
  ON public.orders (customer_id, created_at DESC);

-- ============================================================================
-- 2. Update get_unified_orders — add 5 new customer fields
-- ============================================================================
-- Drop overloads first to avoid ambiguity errors with the Management API
  ...
  IF v_session.identity_type = 'customer' THEN
    RETURN (
      ...
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        -- new customer info fields (all nullable)
...
COMMENT ON FUNCTION public.get_unified_orders IS 'قائمة موحدة للطلبات — مع customer_owner, created_by_name, customer_stats';

-- ============================================================================
-- 3. Update get_unified_order — add 5 new customer fields
-- ============================================================================
...
COMMENT ON FUNCTION public.get_unified_order IS 'مصدر الحقيقة الموحد للطلب — مع customer_stats و Signature واحد: (p_token text, p_id uuid)';
