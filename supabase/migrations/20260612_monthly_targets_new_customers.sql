-- ============================================================================
-- Monthly Targets System — Add New Customers Target
-- Adds new_customers_target and new_customers_weight_percent to both tables
-- Updates all 6 RPCs with new_customers calculation and weight validation
-- ============================================================================

-- 1. Add columns to company_monthly_targets -----------------------------------
ALTER TABLE IF EXISTS public.company_monthly_targets
    ADD COLUMN IF NOT EXISTS new_customers_target numeric NOT NULL DEFAULT 0;

ALTER TABLE IF EXISTS public.company_monthly_targets
    ADD COLUMN IF NOT EXISTS new_customers_weight_percent numeric NOT NULL DEFAULT 25;

COMMENT ON COLUMN public.company_monthly_targets.new_customers_target IS 'هدف عدد العملاء الجدد';
COMMENT ON COLUMN public.company_monthly_targets.new_customers_weight_percent IS 'وزن العملاء الجدد في التقييم';

-- 2. Add columns to employee_monthly_targets ----------------------------------
ALTER TABLE IF EXISTS public.employee_monthly_targets
    ADD COLUMN IF NOT EXISTS new_customers_target numeric NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.employee_monthly_targets.new_customers_target IS 'هدف عدد العملاء الجدد';

-- 3. Drop old functions (signatures changed) ----------------------------------
...
DROP FUNCTION IF EXISTS public.get_upper_management_dashboard CASCADE;

-- 4. RPC: get_governed_company_monthly_target ---------------------------------
...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'new_customers_weight_percent', t.new_customers_weight_percent,
...
-- 5. RPC: governed_upsert_company_monthly_target ------------------------------
CREATE OR REPLACE FUNCTION public.governed_upsert_company_monthly_target(
    ...
    p_sales_weight_percent numeric DEFAULT 40,
    p_visits_weight_percent numeric DEFAULT 20,
    p_orders_weight_percent numeric DEFAULT 20,
    p_new_customers_weight_percent numeric DEFAULT 20
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 6. RPC: get_governed_employee_monthly_targets -------------------------------
...
-- 7. RPC: governed_upsert_employee_monthly_target -----------------------------
...
-- 8. RPC: get_governed_target_performance -------------------------------------
...
DECLARE
    ...
    v_company_new_customers_pct numeric;
...
BEGIN
    ...
    -- Company new customers this month (first delivered order falls in this month)
    SELECT COUNT(*)::int
    INTO v_company_new_customers
    ...
    v_company_orders_pct := CASE WHEN v_company_target.orders_target > 0
        ...
        ELSE 0 END;
    v_company_new_customers_pct := CASE WHEN v_company_target.new_customers_target > 0
        THEN ROUND((v_company_new_customers::numeric / v_company_target.new_customers_target * 100)::numeric, 2)
        ELSE 0 END;

    v_company_overall := ROUND(
        (v_company_sales_pct * v_sales_weight / 100) +
        (v_company_visits_pct * v_visits_weight / 100) +
        (v_company_orders_pct * v_orders_weight / 100) +
        (v_company_new_customers_pct * v_new_customers_weight / 100), 2
    );
    ...
    v_company_info := jsonb_build_object(
        ...
        'new_customers_weight_percent', v_new_customers_weight,
        ...
        'new_customers_achievement_pct', v_company_new_customers_pct,
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'new_customers_achievement_pct', CASE WHEN ec.new_customers_target > 0
                THEN ROUND((ec.new_customers_actual::numeric / ec.new_customers_target * 100)::numeric, 2)
                ELSE 0 END,
            'overall_achievement_score', ROUND(
                ...
                (CASE WHEN ec.new_customers_target > 0 THEN ROUND((ec.new_customers_actual::numeric / ec.new_customers_target * 100)::numeric, 2) ELSE 0 END * v_new_customers_weight / 100)
            , 2),
...
-- 9. Update get_upper_management_dashboard with new_customers KPIs ------------
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- New customers: first delivered order falls in this month
    SELECT COUNT(*)::int INTO v_new_customers_achieved
    FROM public.customers c
    WHERE EXISTS (
        SELECT 1 FROM public.orders o
        WHERE o.customer_id = c.id
    ...
    v_overall := ROUND(
        ...
        (v_orders_pct * v_company_target.orders_weight_percent / 100) +
        (v_new_customers_pct * v_company_target.new_customers_weight_percent / 100), 2
    );
    ...
    employee_new_customers AS (
        ...
        WHERE eo.delivered_at = (
            SELECT MIN(o2.delivered_at)
            FROM public.orders o2
            WHERE o2.customer_id = eo.customer_id AND o2.status = 'delivered'
        )
        GROUP BY eo.employee_id
    ),
    employee_returns AS (
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'overall_achievement_score', ROUND(
                ...
                (CASE WHEN ec.new_customers_target > 0 THEN ROUND((ec.new_customers_actual::numeric / ec.new_customers_target * 100)::numeric, 2) ELSE 0 END * v_company_target.new_customers_weight_percent / 100)
            , 2)
        )
        ORDER BY
            (CASE WHEN ec.sales_target > 0 THEN ROUND((ec.effective_sales / ec.sales_target * 100)::numeric, 2) ELSE 0 END * v_company_target.sales_weight_percent / 100) +
