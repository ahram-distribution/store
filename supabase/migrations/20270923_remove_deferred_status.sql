-- ============================================================================
-- Migration: Remove "deferred" order status
--
-- Final active workflow: submitted → reviewing → approved → preparing → prepared → dispatched → delivered
-- ============================================================================

-- 1. Update execution_status_group() — no change needed (deferred was never in it)

-- 2. Update governed_change_order_status() — remove deferred from statuses array + exceptional logic
...
BEGIN
  ...
  -- Reference number rule — mandatory when entering reviewing
...
COMMENT ON FUNCTION public.governed_change_order_status IS 'تغيير حالة الطلب مع التحقق من الصلاحيات وقواعد الانتقال (v4 — removes deferred; final workflow: submitted→reviewing→approved→preparing→prepared→dispatched→delivered)';
