-- Migration: Update get_live_workday_overview to include visit_count, break_count, net_minutes
-- For active employees in employee_summary
...
BEGIN
    ...
    active_breaks AS (
        SELECT wb.session_id, wb.employee_id, wb.break_start
    ...
    employee_summary AS (
        SELECT
            ...
            (SELECT COUNT(*) FROM public.visit_links vl2 WHERE vl2.session_id = as2.id) AS visit_count,
            (SELECT COUNT(*) FROM public.workday_breaks wb2 WHERE wb2.session_id = as2.id AND wb2.break_end IS NOT NULL) AS completed_break_count,
            (SELECT COALESCE(SUM(COALESCE(duration_seconds, 0)), 0) FROM public.workday_breaks wb3 WHERE wb3.session_id = as2.id AND wb3.break_end IS NOT NULL) AS completed_break_seconds,
            CASE WHEN ab.employee_id IS NOT NULL
                THEN EXTRACT(EPOCH FROM (now() - ab.break_start))::int
                ELSE 0
    ...
    SELECT jsonb_build_object(
        ...
        'employees', COALESCE((SELECT jsonb_agg(jsonb_build_object(
            ...
            'break_count', es.completed_break_count,
            'break_minutes', ((es.completed_break_seconds + es.current_break_seconds) / 60)::int,
            'net_minutes', (es.duration_minutes - (es.completed_break_seconds + es.current_break_seconds) / 60)::int,
            'last_seen_label',
