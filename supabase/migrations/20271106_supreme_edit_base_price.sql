-- ============================================================================
-- 20271106 — Persist BASE unit price on order_items + restore base on removal
-- ----------------------------------------------------------------------------
-- Product requirement: when the discount (tier/payment/shipping) is removed
-- from an order, item unit prices MUST return to the base price (السعر الأساسي).
--
-- Problem: order_items held only ONE price. When a supreme edit applied a
-- discount it wrote the net (discounted) price; removing the discount later
-- could not recover the base price — the UI kept showing net prices.
--
-- Fix:
--   1. Add order_items.base_unit_price (the pre-discount unit price).
--   2. Backfill existing rows: base = current unit price (safe default).
--   3. Detect legacy rows that hold a "clean" catalog-discount of the product
--      price (_calc_base_unit_price) and persist the catalog price as base.
--   4. Orders that currently have discount_amount = 0 but whose items still
--      carry net (base != unit) prices are restored to base prices and their
--      subtotal/total recomputed (discount stays 0).
--   5. get_unified_order now returns base_unit_price per item so the edit
--      screen edits the base price (stable across repeated discount changes).
--
-- The supreme edit RPC (governed_supreme_edit_order_v2) is updated separately
-- to always store both base_unit_price and the final unit_price.
-- ============================================================================

-- 1. Column + safe default backfill.
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS base_unit_price numeric(12,2) NOT NULL DEFAULT 0;

UPDATE public.order_items SET base_unit_price = unit_price WHERE base_unit_price = 0;

-- 2. Detect rows whose stored unit_price is a "clean" discount of the current
--    catalog unit price and persist that catalog price as the base.
--    Custom/negotiated supreme-edit prices (which do not match the catalog
--    after rounding) are left untouched.
WITH catalog_base AS (
  SELECT oi.id AS item_id,
         public._calc_base_unit_price(p.carton_price, p.carton_quantity, oi.unit_type) AS cb
  FROM public.order_items oi
  JOIN public.products p ON p.id = oi.product_id
  WHERE p.carton_price IS NOT NULL
), net_detected AS (
  SELECT cb.item_id, cb.cb
  FROM catalog_base cb
  JOIN public.order_items oi ON oi.id = cb.item_id
  WHERE cb.cb > 0
    AND oi.unit_price <> cb.cb
    AND EXISTS (
      SELECT 1 FROM (VALUES (0.5), (1), (1.5), (2), (2.5), (3), (3.5), (4), (5), (7.5), (10), (12.5), (15), (20))
        AS f(pct)
      WHERE ABS(ROUND((cb.cb * (1 - pct / 100))::numeric, 2) - oi.unit_price) < 0.01
    )
)
UPDATE public.order_items oi
SET base_unit_price = nd.cb
FROM net_detected nd
WHERE oi.id = nd.item_id;

-- 3. Orders with NO active discount whose items are still net-baked → restore
--    the base (السعر الأساسي) and recompute the order sums so the products
--    table, print and WhatsApp all show base prices and totals again.
WITH affected AS (
  SELECT DISTINCT oi.order_id
  FROM public.order_items oi
  JOIN public.orders o ON o.id = oi.order_id
  WHERE o.discount_amount = 0
    AND COALESCE(oi.base_unit_price, 0) > 0
    AND oi.base_unit_price <> oi.unit_price
), base_total AS (
  SELECT oi.order_id, SUM(ROUND(oi.base_unit_price * oi.unit_quantity::numeric, 2)) AS tot
  FROM public.order_items oi
  WHERE oi.order_id IN (SELECT order_id FROM affected)
  GROUP BY oi.order_id
)
UPDATE public.order_items oi
SET unit_price    = oi.base_unit_price,
    total_price   = ROUND(oi.base_unit_price * oi.unit_quantity::numeric, 2)
FROM affected a
...
FROM base_total bt
WHERE o.id IN (SELECT order_id FROM affected)
  AND o.id = bt.order_id;

-- 4. get_unified_order — expose base_unit_price per item so the frontend can
--    edit the base price and know whether displayed prices are already net.
...
COMMENT ON FUNCTION public.get_unified_order IS 'مصدر الحقيقة الموحد للطلب — يعرض الطلب مع العميل والأصناف (بما فيها base_unit_price) والتوصيل والتحصيل والمرتجعات';