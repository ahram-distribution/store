-- ============================================================================
-- Migration: Canonical source — add same order-card fields to get_unified_order
--
-- The singular get_unified_order (used by OrderDetailPage) must expose the
-- SAME business calculation as get_unified_orders (used by OrdersPage).
--
-- Changes:
--   1. customer sub-object: add display_address (same concat_ws + snapshot fallback)
--   2. customer sub-object: add LATERAL subquery for previous-order stats
--   3. customer sub-object: add previous_order_count, previous_orders_total,
--      previous_order_number, previous_order_date, previous_order_total
--
-- The LATERAL subquery is IDENTICAL to the one in get_unified_orders (plural).
-- Business logic is never duplicated — it is the same SQL copied verbatim.
-- ============================================================================
