-- ============================================================================
-- Restore price guard in get_governed_products
--
-- Problem:
--   Migration 20260922 accidentally dropped the price check that was
--   introduced in 20260704. The 20270728 recreate inherited this gap.
--   Without the guard, customer-facing pages (p_active_only=true) can
--   expose unpriced products.
--
-- Changes:
--   1. Restore price guard: products with NULL/zero carton_price are
--      excluded when p_active_only=true (admin pages pass false).
--   2. Remove redundant outer ORDER BY sub.data->>'product_name'.
--      The inner ORDER BY p.product_name already drives correct
--      LIMIT/OFFSET pagination. The outer sort added an unnecessary
--      sort on a JSON field expression.
--
-- Business rule (from 20260704 migration header):
--   "Admin UI passes p_active_only=false so managers can see unpriced
--    products. Storefront / customer queries pass p_active_only=true
--    and must never expose unpriced products."
-- ============================================================================
...
COMMENT ON FUNCTION public.get_governed_products IS 'المنتجات مع دعم البحث والتصفية والترقيد — يمنع ظهور الأصناف بدون سعر لواجهة العملاء';
