-- ============================================================================
-- شاشة الحضور والمتابعة — الجزء 7: تفعيل نظام التحكم في حسابات الشاشة
--   إعادة كتابة RPCs الرئيسية لتكون واعيةً بـ:
--     1) نطاق الحضور و نطاق المتابعة (نظام تحكم منفصل، لا يمس طبقة الحضور القديمة).
--     2) نوع الدوام الفعّال (fixed/flexible) وتفعيل/تعطيل حساب التأخير والانصراف
--        المبكر — المرن لا يُصنَّف أبداً (تأخير/مبكر/التزام), والثابت يُحسب ضمن
--        الالتزام بالساعات الرسمية فقط عند تفعيل الحسابين.
--     3) حدود اليوم بالقاهرة: SET timezone محلي لكل دالة + افتراضيات معلمات
--        Cairo — دون أي تغيير عالمي على منطقة قاعدة البيانات.
--     4) أول/آخر نشاط للفترة/اليوم (يزود قسم الحضور للمرن).
--   الوظائف المغطاة: list / kpis / detail / timeline / auto-close / workforce.
-- ============================================================================

-- ============================================================================
-- get_executive_followup_list — القائمة الرئيسية (وعي بنظام التحكم)
-- ============================================================================
...
-- ============================================================================
-- get_executive_overview_kpis — مؤشرات مجمعة واعية بنظام التحكم
-- ============================================================================
...
BEGIN
    ...
    RETURN (
        ...
        staff AS (
            SELECT
                ve.employee_id, ve.code, ve.full_name, ve.is_active,
                p.attendance_enabled, p.follow_up_enabled, p.schedule_type,
            ...
            LEFT JOIN today_breaks tb ON tb.session_id = ls.session_id
            LEFT JOIN last_activity la ON la.employee_id = ve.employee_id
            GROUP BY ve.employee_id, ve.code, ve.full_name, ve.is_active,
                     p.attendance_enabled, p.follow_up_enabled, p.schedule_type,
                     p.late_calculation_enabled, p.early_calculation_enabled,
                     od.order_count, od.sales_value,
                     cd.collection_count, cd.collection_amount,
        ...
        att_staff AS (SELECT * FROM staff WHERE attendance_enabled),
        comp_staff AS (SELECT * FROM staff WHERE attendance_enabled AND schedule_type = 'fixed'),
        ...
        SELECT jsonb_build_object(
            ...
            'control', jsonb_build_object(
                ...
                'follow_up_monitored', (SELECT COUNT(*)::int FROM staff WHERE follow_up_enabled),
                'flexible', (SELECT COUNT(*)::int FROM staff WHERE schedule_type = 'flexible'),
                'fixed', (SELECT COUNT(*)::int FROM staff WHERE schedule_type = 'fixed')
            ),
            'definition_note', 'أقسام التحكم: الحضور للموظفين ضمن نطاق الحضور، الالتزام للدوام الثابت فقط (تأخير/مبكر/غياب/إغلاق تلقائي)، المرن يُظهر ساعات العمل الفعلية ولا يُصنَّف تأخير/مبكر. مؤشر الأداء = صافي المبيعات لكل ساعة حضور فعلية.',
            'kpis', jsonb_build_object(
                'workforce', (SELECT COUNT(*)::int FROM ve),
                'attendance_monitored', (SELECT COUNT(*)::int FROM att_staff),
                'follow_up_monitored', (SELECT COUNT(*)::int FROM staff WHERE follow_up_enabled),
                'flexible_count', (SELECT COUNT(*)::int FROM staff WHERE schedule_type = 'flexible'),
                'fixed_count', (SELECT COUNT(*)::int FROM staff WHERE schedule_type = 'fixed'),
...
-- ============================================================================
-- get_executive_employee_day_detail — تفاصيل يوم الموظف (وعي بالسياسة)
-- ============================================================================
...
-- ============================================================================
-- get_executive_day_timeline — حدود القاهرة فقط (أحداث سجل حقيقي)
-- ============================================================================
...
-- ============================================================================
-- get_executive_auto_close_report — حدود القاهرة فقط
-- ============================================================================
...
-- ============================================================================
-- get_executive_workforce_history — حدود القاهرة + وعي بالسياسة للتأخير/المبكر
-- ============================================================================
