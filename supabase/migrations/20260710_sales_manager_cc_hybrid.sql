-- ============================================================================
-- Sales Manager Command Center RPC — HYBRID
-- المصدر الأساسي: 20260710_sales_manager_cc.sql (هيكل 8 أقسام)
-- الإضافات من النسخة الحالية في DB:
--   • check_capability(p_token, 'attendance.view_team_map')
--   • synced_at بدلاً من recorded_at في tracking_points
--   • فلتر accuracy_meters <= 100
--   • location_interval_seconds الديناميكي من workday_settings
-- الإصلاح:
--   • today_orders CTE: يستخدم orders.created_by + JOIN employees.identity_id
--     بدلاً من الاعتماد على owner_id (الذي أصبح مختلطاً بعد ترحيل 20260605)
-- ============================================================================
...
BEGIN
  ...
  last_points AS (
    ...
    WHERE tp.synced_at >= CURRENT_DATE
      AND (tp.accuracy_meters IS NULL OR tp.accuracy_meters <= 100)
  ...
  today_orders AS (
    ...
    JOIN public.employees e ON e.identity_id = o.created_by
    WHERE o.created_at::date = CURRENT_DATE
      AND e.id = ANY(v_subtree_ids)
    GROUP BY e.id
  ),
  total_breaks AS (
  ...
  LEFT JOIN active_breaks ab ON ab.session_id = as2.session_id
  LEFT JOIN LATERAL (
...
COMMENT ON FUNCTION public.get_sales_manager_cc IS 'مركز قيادة مدير البيع — بيانات الفريق المباشر والأداء والتتبع الحي (HYBRID)';
