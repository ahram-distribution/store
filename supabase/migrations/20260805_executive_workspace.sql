-- =============================================================================
-- Phase 5 — Executive Operations Workspace (محمد عبد الباسط)
-- =============================================================================
-- 1. Fix governed_dispatch_order to accept 'prepared' + external delivery params
-- 2. Create get_governed_executive_kpis
-- 3. Create get_governed_executive_queue
-- 4. Create get_governed_external_carriers
-- =============================================================================

-- =============================================================================
-- 1. Fix governed_dispatch_order
-- =============================================================================
-- Add 'prepared' to accepted statuses, add external delivery parameters
...
BEGIN
  ...
  IF v_old_status NOT IN ('approved', 'prepared', 'ready_for_dispatch', 'sent_to_delivery') THEN
    RETURN jsonb_build_object('error', 'INVALID_STATE');
  END IF;

  -- Set delivery_mode based on which params are provided
...
-- 2. Create get_governed_executive_kpis
-- =============================================================================
CREATE OR REPLACE FUNCTION public.get_governed_executive_kpis(p_token uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'extensions'
...
BEGIN
  ...
  -- Fully collected: delivered orders with collections >= total
  ...
  -- Partially collected: delivered orders with some collections but less than total
...
-- 4. Create get_governed_external_carriers
-- =============================================================================
CREATE OR REPLACE FUNCTION public.get_governed_external_carriers(p_token uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'extensions'
...
BEGIN
  ...
  IF NOT FOUND THEN RETURN '[]'::jsonb; END IF;
  RETURN (
    SELECT COALESCE(jsonb_agg(jsonb_build_object('id', id, 'name', name, 'code', code)), '[]'::jsonb)
    FROM public.external_carriers WHERE is_active = true
  );
END;
$function$;
