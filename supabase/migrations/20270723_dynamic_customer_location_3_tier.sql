-- ================================================================
-- Dynamic Customer Location Resolution (3-tier)
--
-- Changes:
--   1. New function: get_effective_customer_location(uuid)
--      Returns the best available location for any customer using:
--        Priority 1: customer_location (unified_locations)
--        Priority 2: visit_gps (latest completed visit check-out GPS)
--        Priority 3: address_geocoded (geocoded from customer_addresses)
--
--   2. Updated: get_coverage_map — 3-tier instead of 5-tier
--      Removed: city_center, governorate_center as location sources
--      Renamed: gps → customer_location
-- ================================================================

-- ================================================================
-- 1. get_effective_customer_location — reusable 3-tier resolver
-- ================================================================

CREATE OR REPLACE FUNCTION public.get_effective_customer_location(p_customer_id uuid)
RETURNS TABLE (
    latitude numeric,
    longitude numeric,
    source text
...
BEGIN
    ...
    WITH customer_loc AS (
        SELECT ul.latitude, ul.longitude, 'customer_location'::text AS source
        FROM public.customers c
        JOIN public.unified_locations ul ON ul.id = c.location_id
        WHERE c.id = p_customer_id AND ul.latitude IS NOT NULL
    ),
    visit_loc AS (
        SELECT v.check_out_latitude, v.check_out_longitude, 'visit_gps'::text AS source
        FROM public.visits v
        WHERE v.customer_id = p_customer_id
          AND v.status = 'completed'
          AND v.check_out_latitude IS NOT NULL
          AND v.check_out_longitude IS NOT NULL
        ORDER BY v.check_out_at DESC
        LIMIT 1
    ),
    address_loc AS (
        SELECT caddr.latitude, caddr.longitude, 'address_geocoded'::text AS source
        FROM public.customers c
        LEFT JOIN LATERAL fn_customer_default_address(c.id) caddr ON true
        WHERE c.id = p_customer_id AND caddr.latitude IS NOT NULL
    )
    SELECT * FROM customer_loc
    UNION ALL
    SELECT * FROM visit_loc WHERE NOT EXISTS (SELECT 1 FROM customer_loc)
    UNION ALL
    SELECT * FROM address_loc
    WHERE NOT EXISTS (SELECT 1 FROM customer_loc)
      AND NOT EXISTS (SELECT 1 FROM visit_loc)
    LIMIT 1;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_effective_customer_location TO anon;
GRANT EXECUTE ON FUNCTION public.get_effective_customer_location TO authenticated;

COMMENT ON FUNCTION public.get_effective_customer_location IS
  'تحديد أفضل موقع متاح للعميل ديناميكياً حسب الأولوية: موقع العميل ← زيارة GPS ← ترميز جغرافي';

-- ================================================================
-- 2. get_coverage_map — updated to 3-tier priority
-- ================================================================
...
BEGIN
    ...
    -- ============================================================
    -- CUSTOMER LOCATIONS — 3-tier priority
    --   1. customer_location:  unified_locations (stored GPS)
    --   2. visit_gps:          latest completed visit check-out GPS
    --   3. address_geocoded:   geocoded from customer_addresses
    -- ============================================================
...
COMMENT ON FUNCTION public.get_coverage_map IS
  'خريطة التغطية — 3 مستويات: موقع العميل ← زيارة GPS ← ترميز جغرافي. يتم تحديد الموقع ديناميكياً.';
