-- ============================================================================
-- PHASE A — PERMANENT DELETION WORKFLOW (Business Decision, Final)
-- ============================================================================
-- Contract: deleting an order permanently removes the order and EVERYTHING it
-- owns. Not soft-delete, not archive, no hidden record. After deletion no
-- record may reference the order and the delete must not raise an FK violation.
--
-- Required order of operations:
--   1. Restore inventory first if the order belongs to the Execution State
--      Group (deducted stock must come back exactly once).
--   2. Remove every dependent record (children before parents).
--   3. Delete the order itself.
--   4. Leave zero orphans; never FK-fail.
--
-- Fixes:
...
--     so it also cannot FK-fail or leave orphans.
-- All three are CREATE OR REPLACE, safe to re-apply.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) governed_delete_order — legacy single-order delete (completed dependents)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2) governed_supreme_delete_cancelled_order — add inventory_movements cleanup
--    (fixes inventory_movements_order_id_fkey) and complete the dependent set.
-- ---------------------------------------------------------------------------
...
--    restore the Execution State Group (not just 'approved'), and add the
--    missing auction_awards cleanup so bulk delete cannot FK-fail either.
-- ---------------------------------------------------------------------------
