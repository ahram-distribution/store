  ...
  IF v_session.employee_id IS NOT NULL THEN
    v_authorized := EXISTS (
      ...
      SELECT 1 FROM orders WHERE execution_location_id = p_id
        AND (public.resolve_employee_id(created_by) = v_session.employee_id OR owner_id = v_session.employee_id)
...
BEGIN
    ...
    session_kpis AS (
        ...
        ) cd ON cd.created_by = wds.employee_id AND cd.d = wds.date
        LEFT JOIN (
