-- ============================================================================
-- Supreme Management Full Order Control
-- Business Decision (Approved):
-- Supreme Management / Super Admin can edit ANY order (any status),
-- override prices, add/remove items, and delete cancelled orders.
-- All changes are audited via order_modification_history.
-- ============================================================================

-- ============================================================================
-- Helper: check if employee is Supreme Management or Super Admin
-- ============================================================================
...
AS $$
  SELECT EXISTS(
    ...
    WHERE er.employee_id = p_employee_id
      AND r.name IN ('سوبر أدمن', 'SUPER_ADMIN', 'رئيس مجلس الإدارة', 'أدمن', 'ADMIN')
  );
$$;

-- ============================================================================
-- RPC: governed_supreme_edit_order
--
-- Supreme Management can edit ANY order regardless of status.
-- Replaces all order items atomically, records audit trail with full diffs,
-- recalculates totals, and preserves the original order status.
-- ============================================================================
...
BEGIN
  ...
  -- Only Supreme Management / Super Admin
  v_is_super := public.is_supreme_management(v_session.employee_id);
  IF NOT v_is_super THEN
    RETURN jsonb_build_object('error', 'FORBIDDEN', 'detail', 'Only Supreme Management and Super Admin can edit orders');
  END IF;
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      -- Re-insert old items on failure for atomicity? No — we roll back automatically.
  ...
  -- Audit trail: Write order_modification_history with full item diffs
...
COMMENT ON FUNCTION public.governed_supreme_edit_order IS
  'Supreme Management / Super Admin: Edit ANY order (any status). Replaces all items, records audit trail with full diffs.';

-- ============================================================================
-- RPC: governed_supreme_delete_cancelled_order
--
-- Supreme Management can permanently delete cancelled orders.
-- Only works when order status is 'cancelled'.
-- Restores inventory, releases credit, cleans up related records.
-- ============================================================================
...
BEGIN
  ...
  v_is_super := public.is_supreme_management(v_session.employee_id);
  IF NOT v_is_super THEN
    RETURN jsonb_build_object('error', 'FORBIDDEN', 'detail', 'Only Supreme Management and Super Admin can delete cancelled orders');
  END IF;
  ...
  -- Delete package_orders
  DELETE FROM public.package_orders WHERE order_id = p_order_id;
  ...
  -- Delete order_daily_deals, order_flash_offers (no CASCADE)
...
COMMENT ON FUNCTION public.governed_supreme_delete_cancelled_order IS
  'Supreme Management / Super Admin: Permanently delete a cancelled order with full cleanup.';
