-- ============================================================================
-- 0005: get_unified_orders — date source selector (creation time vs last
--      operational event)
--
-- Adds an optional 10th parameter:
--   p_date_source text DEFAULT 'created'
--
--   'created' (default) — existing behavior, the date range filters by
--       o.created_at (order creation time). Byte-for-byte the same WHERE
--       conditions as before.
--
--   'event' — the same date range filters by the LATEST REAL operational
--       event recorded for the order, taken exclusively from the existing
--       Order Details "سجل الأحداث" timeline sources (buildTimelineEvents):
--         orders.created_at                            (order creation)
--         order_status_history.changed_at              (status changes)
--         order_modification_history.modified_at       (order edits)
--         preparation_records started_at/completed_at  (latest record, preparation)
--         delivery_tracking  assigned_at/started_at/completed_at (delivery)
--         collections.collected_at (status <> 'pending', order + customer-wide) (collections)
--         returns.created_at                           (returns)
--       Orders with no recorded events fall back to o.created_at.
--       No updated_at substitution, no new event system, no new timestamp.
--
-- The event expression is evaluated only when p_date_source = 'event'
-- (LATERAL join ON p_date_source = 'event'), so the default mode incurs no
-- extra work and returns exactly the same rows as before.
-- ============================================================================

DROP FUNCTION IF EXISTS public.get_unified_orders(uuid, text, varchar, uuid, uuid, timestamptz, timestamptz, uuid, boolean);
  ...
  RETURN (
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'last_event_ts', COALESCE(lev.last_event_ts, o.created_at)
      )
      ORDER BY COALESCE(lev.last_event_ts, o.created_at) DESC, o.created_at DESC
    ), '[]'::jsonb)
    ...
    LEFT JOIN LATERAL (
      SELECT CASE WHEN p_date_source = 'event' THEN GREATEST(
        COALESCE((SELECT max(changed_at) FROM public.order_status_history osh WHERE osh.order_id = o.id), '-infinity'::timestamptz),
        COALESCE((SELECT max(modified_at) FROM public.order_modification_history omh WHERE omh.order_id = o.id), '-infinity'::timestamptz),
        COALESCE((SELECT GREATEST(COALESCE(pr.started_at, '-infinity'::timestamptz), COALESCE(pr.completed_at, '-infinity'::timestamptz)) FROM public.preparation_records pr WHERE pr.order_id = o.id ORDER BY pr.created_at DESC LIMIT 1), '-infinity'::timestamptz),
        COALESCE((SELECT max(GREATEST(COALESCE(dt.assigned_at, '-infinity'::timestamptz), COALESCE(dt.started_at, '-infinity'::timestamptz), COALESCE(dt.completed_at, '-infinity'::timestamptz))) FROM public.delivery_tracking dt WHERE dt.order_id = o.id), '-infinity'::timestamptz),
        COALESCE((SELECT max(col.collected_at) FROM public.collections col WHERE (col.order_id = o.id OR (col.customer_id = o.customer_id AND col.order_id IS NULL)) AND col.collected_at IS NOT NULL AND col.status <> 'pending'), '-infinity'::timestamptz),
        COALESCE((SELECT max(r.created_at) FROM public.returns r WHERE r.order_id = o.id), '-infinity'::timestamptz)
    ...
    WHERE (v_is_super OR c.owner_id = ANY(v_visible))
      ...
      AND (p_date_from IS NULL OR COALESCE(lev.last_event_ts, o.created_at) >= p_date_from)
      AND (p_date_to IS NULL OR COALESCE(lev.last_event_ts, o.created_at) <= p_date_to)
...
-- No GRANT statements: the `authenticated` role does not exist locally.
-- Web parity grant lives in the supabase migration.