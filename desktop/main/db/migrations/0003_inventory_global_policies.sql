-- ============================================================================
-- Migration 0003 - Inventory global policies (Web parity)
-- ============================================================================
-- Port of supabase/migrations/20270803_inventory_global_policies.sql so the
-- Desktop renderer's ProductManagerPage calls resolve against the LOCAL schema:
--   get_inventory_policies
--   set_global_negative_selling_policy
--   set_global_inventory_deduction_status
--
-- All dependencies already exist in the local baseline (schema.sql):
--   app.app_settings, orders.order_negative_selling_allowed,
--   orders.order_inventory_deduction_status, governed_inventory_restore,
--   check_capability, order_status_history, order_items, products, inventory.
--
-- Adapted for local use:
--   - No GRANT EXECUTE ... TO authenticated (role does not exist locally;
--     the desktop app connects as the app role).
--   - No NOTIFY pgrst.
-- Idempotent: CREATE OR REPLACE / ON CONFLICT DO NOTHING / DROP TRIGGER IF EXISTS.
-- ============================================================================
