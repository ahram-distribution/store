-- ============================================================================
-- Migration 0002 - get_unified_orders with strict previous-order statistics
-- ============================================================================
-- Adds `p_include_strict_previous boolean DEFAULT false` as the 9th argument.
...
--
-- Derived from the local baseline function body (schema.sql, 2026-07-27) plus a
-- strict time-bound lateral (o3.created_at < o.created_at). This matches the
-- Orders report definition of "previous orders" (قبل الطلب الحالي).
--
-- Idempotent: the 8-arg signature is dropped first, then the 9-arg function is
-- created with CREATE OR REPLACE, so re-running is safe.
