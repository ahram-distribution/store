-- Desktop migration 17: Fix customer phone normalization in governed_update_customer
-- Applied to production as the effective definition (CREATE OR REPLACE).
--
-- ROOT CAUSE: migration 0012's normalization used
--   translate(p_phone, '٠١٢٣٤٥٦٧٨٩۰۱۲۳۴۵۶۷۸۹0', '01234567890123456789')
--   --  from is 21 chars, to is 20 chars  --
-- The trailing ASCII '0' in `from` has no `to` counterpart, and PostgreSQL
-- translate() DELETES every input character that matches an unmapped `from`
-- position. As a result every ASCII '0' in a phone number was stripped:
--   '01202107113' -> '12217113'  (8 digits) -> fails ^01[0-9]{9}$
-- causing a false "رقم الهاتف غير صالح" for valid Egyptian mobile numbers.
--
-- FIX (minimal, preserves the exact business rule): balance the mapping by
-- removing the orphan ASCII '0' from the source string. Now:
--   Arabic-Indic digits      -> ASCII digits
--   Eastern Arabic/Persian   -> ASCII digits
--   ASCII digits (incl. '0') -> left unchanged
-- The subsequent regexp_replace('[^0-9]','','g') still strips non-digits,
-- and the validation rule ^01[0-9]{9}$ is unchanged.
--
-- All other behavior of governed_update_customer is preserved exactly.
