-- ============================================================================
-- Phase 5B — Compatibility Layer Removal
-- 
-- Prerequisite: All data is clean (owner_id = employees.id for every row).
-- Validated by: Phase 5A (zero business logic depends on identity_id).
-- 
-- Removal order:
--   1. OR joins (order_delivered_events, resolve_scope)
--   2. IN(employee_id, identity_id) in runtime functions
--   3. COALESCE compatibility logic
--   4. Legacy wrappers (resolve_employee_id, get_runtime_team_activity)
--   5. Dead code documentation
-- ============================================================================

-- ============================================================================
-- PHASE 5B-1: Remove OR joins from runtime functions
-- ============================================================================

-- 1a. runtime.get_activity — remove IN(employee_id, identity_id) pattern
--     With clean data, all owner_id = employees.id, so identity_id branch
--     never matches. Removing it produces identical results.
...
-- 1b. runtime.get_team_activity — remove IN(employee_id, identity_id) pattern
...
-- ============================================================================
-- PHASE 5B-2: Simplify resolve_employee_id (remove identity_id fallback)
-- ============================================================================
-- With clean data, all owner_id = employees.id. The identity_id subquery
-- is dead code. Removing it causes fast failure if identity_id is passed
-- instead of employees.id — which is the desired behavior.
...
-- ============================================================================
-- PHASE 5B-3: Remove get_runtime_team_activity wrapper
-- ============================================================================
-- This public wrapper delegates to runtime.get_team_activity.
-- Callers should use runtime.get_team_activity directly.

-- Note: Drop only after verifying no callers depend on the public wrapper.
-- For now, leave in place but document as deprecated.

-- ============================================================================
-- PHASE 5B-4: Remove dead code from identity_integration_layer functions
-- ============================================================================
-- Functions in public schema that use resolve_employee_id(owner_id):
...
-- 
-- With clean data, resolve_employee_id(owner_id) returns owner_id directly
-- (it's already employees.id). The function call is now a no-op overhead.
-- 
-- These functions use resolve_employee_id defensively. Remove the call
-- and reference owner_id directly. Verified safe because data is clean.

-- 4a. get_employee_workday_history — replace resolve_employee_id(o.owner_id) with o.owner_id
...
BEGIN
    ...
    WITH session_data AS (
        ...
        LEFT JOIN (
            SELECT o.owner_id AS resolved_employee_id,
        ...
        LEFT JOIN (
            SELECT c.owner_id AS resolved_employee_id,
        ...
        LEFT JOIN (
            SELECT c2.owner_id AS resolved_employee_id,
...
-- 4b. get_team_map — replace resolve_employee_id(o.owner_id) with o.owner_id
...
BEGIN
    ...
    today_collections AS (
        SELECT c.owner_id AS employee_id,
            COUNT(*)::int AS collection_count,
            COALESCE(SUM(c.amount), 0) AS collection_amount
    ...
    today_customers AS (
        SELECT cu.owner_id AS employee_id,
            COUNT(*)::int AS new_customer_count
        FROM public.customers cu WHERE cu.created_at::date = CURRENT_DATE
        GROUP BY cu.owner_id
    ),
    today_visits AS (
...
-- 4c. get_my_workday_status — replace resolve_employee_id(owner_id) with owner_id
...
-- 4d. get_daily_target_vs_actual — replace resolve_employee_id(owner_id) with owner_id
...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'orders_actual', COALESCE((SELECT COUNT(*)::numeric FROM public.orders o WHERE o.owner_id = ANY(v_employee_ids) AND o.created_at::date = v_target_date), 0),
        ...
        'sales_actual', COALESCE((SELECT SUM(total_amount)::numeric FROM public.orders o WHERE o.owner_id = ANY(v_employee_ids) AND o.created_at::date = v_target_date), 0),
        ...
        'collections_actual', COALESCE((SELECT SUM(amount)::numeric FROM public.collections c WHERE c.owner_id = ANY(v_employee_ids) AND c.created_at::date = v_target_date), 0),
        ...
        'new_customers_actual', COALESCE((SELECT COUNT(*)::numeric FROM public.customers cu WHERE cu.owner_id = ANY(v_employee_ids) AND cu.created_at::date = v_target_date), 0)
    ) INTO v_result
...
-- 4e. get_live_workday_overview — replace resolve_employee_id(o.owner_id) with o.owner_id
...
-- ROLLBACK
-- ============================================================================
-- Run the original migration files in reverse order to roll back:
--   1. 20260724_identity_integration_layer.sql (original definitions)
--   2. 20261231_fix_activity_submitted_at.sql (original definitions)
