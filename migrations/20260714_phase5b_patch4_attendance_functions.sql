-- ============================================================================
-- PATCH 4 of 4: Attendance Functions — Remove resolve_employee_id wrappers
--
-- Functions modified:
--   1. get_employee_workday_history
--   2. get_team_map
--   3. get_my_workday_status
--   4. get_daily_target_vs_actual
--   5. get_live_workday_overview
--
-- Why necessary:   All 5 functions use resolve_employee_id(o.owner_id) in
--                  their CTEs to handle the case where owner_id might be
--                  employees.identity_id instead of employees.id. With zero
--                  contaminated rows, resolve_employee_id(o.owner_id) always
--                  returns o.owner_id. The function call is dead overhead.
--
-- What it fixes:   Eliminates the resolve_employee_id indirection from 5
--                  production attendance and monitoring functions. Direct
--                  owner_id references are faster and make the data contract
--                  explicit.
--
-- Depends on:      Patches 1, 2, 3 complete and verified.
--                  Data clean (verified: 0 contaminated rows).
--
-- Risk level:      LOW. All 5 use CREATE OR REPLACE FUNCTION. Each change
--                  is: resolve_employee_id(o.owner_id) → o.owner_id.
--                  With clean data, results are IDENTICAL.
--
-- Rollback:        Re-run the original function definitions from their source
--                  migration files. See ROLLBACK section at end.
--
-- Regression:      Before:  Record KPI totals for attendance dashboard
...
-- ============================================================================
-- 1. get_employee_workday_history — Replace resolve_employee_id with direct
-- ============================================================================
...
BEGIN
    ...
    WITH session_data AS (
        ...
        LEFT JOIN (
            -- CHANGED: resolve_employee_id(o.owner_id) → o.owner_id
        ...
        LEFT JOIN (
            -- CHANGED: resolve_employee_id(c.owner_id) → c.owner_id
        ...
        LEFT JOIN (
            -- CHANGED: resolve_employee_id(c2.owner_id) → c2.owner_id
...
-- ============================================================================
-- 2. get_team_map — Replace resolve_employee_id with direct
-- ============================================================================
...
BEGIN
    ...
    today_customers AS (
        -- CHANGED: resolve_employee_id(cu.owner_id) → cu.owner_id
...
-- ============================================================================
-- 3. get_my_workday_status — Replace resolve_employee_id with direct
-- ============================================================================
...
BEGIN
    ...
    -- CHANGED: resolve_employee_id(o.owner_id) → o.owner_id (all three queries)
...
-- ============================================================================
-- 4. get_daily_target_vs_actual — Replace resolve_employee_id with direct
-- ============================================================================
...
BEGIN
    ...
    -- CHANGED: All resolve_employee_id calls replaced with direct owner_id in aggregate queries
    SELECT jsonb_build_object(
...
-- ============================================================================
-- 5. get_live_workday_overview — Replace resolve_employee_id with direct
-- ============================================================================
...
-- ROLLBACK
-- ============================================================================
-- To roll back ALL 5 functions, re-run these migrations in order:
--   1. 20260610_attendance_module.sql (original definitions)
--   2. 20260724_identity_integration_layer.sql (identity integration versions)
--   3. 20260802_phase_c_followup_schedule_type_rpcs.sql (latest schedule-aware versions)
--
-- To roll back an INDIVIDUAL function, re-run the specific CREATE OR REPLACE
-- from the migration file that most recently modified it before this patch.
-- ============================================================================
